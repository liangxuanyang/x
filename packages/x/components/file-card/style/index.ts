    ...
    [componentCls]: {
      ...
      [`${componentCls}-file`]: {
        ...
        [`${componentCls}-file-content`]: {
          flex: 1,
          maxWidth: calc("100%")
            .sub(calc(fontSizeHeading1).sub(2).equal())
            .sub(marginSM)
            .equal(),
        },
...
const genFileCardListStyle: GenerateStyle<FileCardToken> = token => {
  ...
  return {
    [`${componentCls}-list`]: {
      ...
      [`${componentCls}-list-motion-enter-active, ${componentCls}-list-motion-leave-active`]:
        {
          transition: `opacity ${motionDurationSlow}`,
...
export default genStyleHooks(
  ...
  token => {
    ...
    return [
      genFileCardStyle(fileCardToken),
      genFileCardListStyle(fileCardToken),
    ];
  },
