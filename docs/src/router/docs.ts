...
const localePattern = new RegExp(
  `\\.(${SUPPORTED_LOCALES.map(escapeRegExp).join("|")})\\.md$`,
);
...
const docRouteInfos = Object.entries(PAGE_MODULES).reduce<DocRouteInfo[]>(
  (routes, [filePath]) => {
    if (
      !filePath.startsWith(PAGE_FILE_PREFIX) ||
      !filePath.endsWith(PAGE_FILE_EXTENSION)
    )
      return routes;
...
const docRouteInfoByBaseAndLocale = new Map(
  docRouteInfos.map(routeInfo => [
    getDocRouteKey(routeInfo.basePath, routeInfo.locale),
    routeInfo,
  ]),
);
...
export function resolveDocRoutePath(path: string, locale: string) {
  ...
  return (
    docRouteInfoByBaseAndLocale.get(
      getDocRouteKey(currentRoute.basePath, locale),
    )?.routePath ?? null
  );
}
